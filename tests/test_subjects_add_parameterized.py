...
class SubjectsAddParameterizedPackage(unittest.TestCase):
    ...
    def test_add_subjects_positive_expand(self, id_student, name, surname, name_subject, expected):
        self.assertEqual(self.tmp.addSubject(id_student, name, surname, name_subject), expected)

    @parameterized.expand([
        (12, "Zosia", "Kucharska", "math", "There_is_not_such_student"),
    ])
    def test_add_subjects_exceptions_expand(self, id_student, name, surname, name_subject, expected):
        self.assertRaisesRegex(Exception, expected, self.tmp.addSubject, id_student, name, surname, name_subject)

    @parameterized.expand([
        (2, "Beata", "Jankowska", True, "Bad_type_subject_name"),
    ])
    def test_add_subjects_type_errors(self, id_student, name, surname, name_subject, expected):
        self.assertRaisesRegex(TypeError, expected, self.tmp.addSubject, id_student, name, surname, name_subject)
